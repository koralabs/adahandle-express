...
describe('Session Tests', () => {
    ...
    it('should send a successful 200 response', async () => {
        ...
        jest.spyOn(jwt, 'verify')
            ...
            // @ts-ignore
            .mockReturnValueOnce({ handle: validHandle, emailAddress: '+1234567890', cost: 10 });
        jest.spyOn(walletHelper, 'getNewAddress').mockResolvedValue(validAddress);
        jest.spyOn(ActiveSessions, 'getActiveSessionsByEmail').mockResolvedValue([]);
    ...
    describe('SPO tests', () => {
        it('Should send 200 successful response', async () => {
            ...
            jest.spyOn(jwt, 'verify')
                ...
                .mockReturnValueOnce('valid')
                // @ts-ignore
                .mockReturnValueOnce({ handle: validHandle, emailAddress: '+1234567890', cost: 250, isSPO: true });
            jest.spyOn(StakePools, 'getStakePoolsByTicker').mockResolvedValue([
                new StakePool({
                    id: '1',
                    ticker: validHandle,
                    stakeKey: 'stakeKey_1',
                    ownerHashes: ['owner1', 'owner2'],
                    isOG: false
                })
            ]);
