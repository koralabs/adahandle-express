...
const mintPaidSessions = async (availableWallet: MintingWallet, state: State): Promise<MintSessionsResponse> => {
    ...
    const getLogMessage = (startTime: number, recordCount: number) => ({
        message: `mintPaidSessions processed ${recordCount} records in ${Date.now() - startTime}ms`,
        event: 'mintPaidSessions.run',
        count: recordCount,
    ...
    Logger.log({
        message: `Found paid sessions: ${JSON.stringify(paidSessions)}`,
        event: 'mintPaidSessionsHandler.foundPaidSessions',
    ...
    if (results.some((result) => !result)) {
        Logger.log({
            ...
            event: 'mintPaidSessionsHandler.updateSessionStatuses.processing',
    ...
    await awaitForEach(paidSessions, async (session: ActiveSession) => {
        ...
        if (existsOnChain || existingSessions.filter((s) => s.status === Status.PAID).length > 1 || inMintingCache) {
            Logger.log({
                message: `Handle ${session.handle} already exists on-chain or in DB`,
                event: 'mintPaidSessionsHandler.handleExists',
                category: LogCategory.NOTIFY
            });
    ...
    if (refundableSessions.length > 0) {
        const items = refundableSessions.map(
            (s) =>
                new ActiveSession({
                    ...s,
                    refundAmount: s.cost,
    ...
    if (sanitizedSessions.length < 1) {
        Logger.log({
            ...
            event: 'mintPaidSessionsHandler.mintHandlesAndSend',
            category: LogCategory.INFO
        });
        return {
            status: 200,
    ...
    Logger.log({
        message: `Attempting to mint: ${JSON.stringify(sanitizedSessions)}`,
        event: 'mintPaidSessionsHandler.attemptingToMint',
        category: LogCategory.INFO
    });
    ...
    try {
        ...
        Logger.log({
            message: `Minted batch with transaction ID: ${txId}`,
            event: 'mintPaidSessionsHandler.mintHandlesAndSend'
        });
        Logger.log({
            message: `Submitting ${sanitizedSessions.length} minted Handles for confirmation.`,
            event: 'mintPaidSessionsHandler.mintHandlesAndSend',
        ...
        Logger.log({
            ...
            event: 'mintPaidSessionsHandler.mintHandlesAndSend.error',
    ...
    try {
        ...
        await ActiveSessions.updateWorkflowStatusAndTxIdForSessions(
            txId,
            walletId,
            sanitizedSessions,
        ...
        Logger.log({
            message: `Post-minting processing failed: ${JSON.stringify(e)}`,
            category: LogCategory.ERROR,
            event: 'mintPaidSessions.postMintProcessing'
        });
        return {
            status: 500,
            error: true,
...
export const mintPaidSessionsHandler = async (req: express.Request, res: express.Response) => {
    ...
    try {
        ...
        Logger.log({
            message: `Found ${JSON.stringify(availableWallet)}`,
            event: 'mintPaidSessionsHandler.foundAvailableWallet',
            category: LogCategory.INFO
        });
        ...
        if (!availableWallet) {
            if (await StateData.allMintingWalletsAreLockedWithNoTransactions()) {
                Logger.log({
                    message: 'All wallets are locked with no transaction IDs',
                    event: 'mintPaidSessionsHandler.availableWallet',
                    category: LogCategory.NOTIFY
                });
            } else {
        ...
        if (availableWallet.balance && availableWallet.balance < availableWallet.minBalance) {
            Logger.log({
                message: `Low balance notification for ${availableWallet.id}`,
                event: 'mintPaidSessionsHandler.availableWallet.balance',
                category: LogCategory.NOTIFY
            });
        }
        ...
        if (availableWallet.balance && availableWallet.balance < toLovelace(100)) {
            Logger.log({
                message: `${availableWallet.id} balance is lower than 100 ADA. LOCKING`,
                event: 'mintPaidSessionsHandler.availableWallet.balance',
                category: LogCategory.NOTIFY
            });
...
const backupNftsToS3 = async (sessions: ActiveSession[]) => {
    await asyncForEach<ActiveSession, void>(sessions, async (session) => {
        ...
        readFile(outputSlug, function (err, data) {
            ...
            s3.putObject(
                {
                    Bucket: 'adahandle-nfts',
                    Key: `${session.handle}.jpg`,
                    Body: data,
                    Metadata: { ipfsHash: session.ipfsHash || '' }
                },
                (err, data) => {
                    if (err) throw err;
